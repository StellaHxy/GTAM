...
from modules.datasets import MoleculeNetDataset2D
from modules.models import GNN
...
def model_setup():
    ...
    if args.model_2d == "GIN": # TODO: will fix this later
        model = GNN(args.num_layer, args.emb_dim, evo_config, JK=args.JK, drop_ratio=args.dropout_ratio, gnn_type='GIN')
...
def get_atom_batch(batch):
    ...
    return torch.tensor(atom_batch, dtype=torch.int64)
...
def train(epoch, device, loader, optimizer):
    ...
    for step, batch in enumerate(L):
        ...
        if args.model_2d == "GIN":
            node_repr,_,_ = model(batch)
            molecule_2D_repr = molecule_readout_func(node_repr, batch.batch)

        embeddings = embeddings + molecule_2D_repr.detach().cpu().tolist()
        if graph_pred_linear is not None:
        ...
        embeddings_y = embeddings_y + y.detach().cpu().tolist()
    ...
    # with open("../GTAM_bace.txt", 'a+') as  f:
    #     for x in embeddings:
    #         f.write(str(x))
    #         f.write('\n')

    # with open("../GTAM_bace_y.txt", 'a+') as  f:
    #     for x in embeddings_y:
    #         f.write(str(int(x[0])))
    #         f.write('\n')
...
def eval(device, loader):
    ...
    for batch in L:
        ...
        embeddings = embeddings + molecule_2D_repr.detach().cpu().tolist()
        embeddings_y = embeddings_y + true.detach().cpu().tolist()
