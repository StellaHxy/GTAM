...
from modules.models.IPA import IPA 

import json
from config import args
from modules.datasets import DatasetMD17, MoleculeDataset3DRadius
from modules.models import SchNet, PaiNN
...
def train(device, loader):
    ...
    for step, batch_data in enumerate(L):
        batch_data.atom_batch = get_atom_batch(batch_data)
        ...
        if args.model_3d == "SchNet":
            _, molecule_3D_repr, zij_3d, pair_mask_3D = model(batch_data, return_latent=True)

        elif args.model_3d == "PaiNN":
            _, molecule_3D_repr, zij_3d, pair_mask_3D = model(batch_data.x[:, 0], batch_data.positions, batch_data.radius_edge_index, batch_data.batch, return_latent=True)
        
        elif args.model_3d == "IPA":
            molecule_3D_repr = model(batch_data, is_finetune=True)
...
def eval(device, loader):
    ...
    for batch_data in L:
        ...
        if args.model_3d == "SchNet":
            molecule_3D_repr = model(batch_data, positions, is_finetune=True)
        elif args.model_3d == "PaiNN":
...
def load_model(model, graph_pred_linear, model_weight_file, load_latest=False):
    ...
    model_weight = torch.load(model_weight_file, map_location='cuda:0')
...
if __name__ == "__main__":
    ...
    if args.model_3d == "SchNet":
        ...
        with open(args.IPA_config, 'r', encoding='utf-8') as f_ipa:
            IPA_config = json.loads(f_ipa.read())
            IPA_config = ml_collections.ConfigDict(IPA_config)
            IPA_config = IPA_config.IPA
